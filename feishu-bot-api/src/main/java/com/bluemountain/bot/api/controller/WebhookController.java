...
public class WebhookController {
    ...
    private final UserAutoRegister userAutoRegister;
    ...
    // URL 验证（飞书配置回调地址时的握手）
    //
    // 在飞书开放平台填写回调地址时，飞书会发一个 GET 请求：
    //   GET /webhook/event?challenge=xxx&token=yyy&type=url_verification
    //
    // 你需要返回 challenge 的值，飞书就知道"这个地址是你控制的"。
    // 这跟微信小程序后台配回调 URL 的验证逻辑一样。
    // ============================================================
    @GetMapping("/event")
    ...
    // 接收事件（用户发消息后，飞书推送到这里）
    //
    // 飞书要求 3 秒内返回 200 OK，所以：
    // ① 验签 → ② 返回 200 → ③ 异步处理（当前同步处理，后续优化）
    // ============================================================
    @PostMapping("/event")
    public Object onEvent(HttpServletRequest request) {
        // ---- 第1步：读原始请求体 ----
        String body = readBody(request);
        log.info("POST body: {}", body); // 调试：看飞书到底发了什么

        // ---- URL验证（POST版）：飞书也会发 POST 带 challenge ----
        ...
        WebhookEvent event;
        try {
            event = JSONUtil.toBean(body, WebhookEvent.class);
        } catch (Exception e) {
        ...
        // 飞书消息的 content 字段是 JSON 字符串，需要二次解析
